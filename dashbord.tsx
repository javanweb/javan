...
const Dashboard: React.FC<DashboardProps> = ({ setView, siteContent, setSiteContent, onNavigateFromDashboard }) => {
  ...
  const handleSaveChanges = async (contentToSave: SiteContent): Promise<boolean> => {
    try {
      // Save content to localStorage instead of a backend server.
      ...
      // Check for quota exceeded error which is common with localStorage
      if (error instanceof DOMException && (error.name === 'QuotaExceededError')) {
        alert('خطا: فضای ذخیره‌سازی مرورگر پر است. امکان ذخیره تغییرات وجود ندارد.');
      } else {
